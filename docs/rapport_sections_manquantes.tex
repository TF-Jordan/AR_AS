...
Le projet \concept{AR\_AS} a permis de concevoir, développer et déployer une plateforme complète d'intelligence artificielle pour la recommandation de véhicules et le classement multicritères de livreurs. Ce travail, réalisé dans le cadre du cours d'Administration Réseaux et Systèmes, a mobilisé un large spectre de compétences techniques, depuis le traitement du langage naturel jusqu'au déploiement conteneurisé.

\section*{Bilan des réalisations}

Les objectifs définis en début de projet ont été intégralement atteints :

\begin{enumerate}
    \item \textbf{Analyse de sentiments} : le module d'analyse basé sur \tech{distil-camembert} permet de classifier automatiquement des avis en français avec un temps d'inférence de $\sim$45ms et un mécanisme de fallback garantissant la disponibilité du service.

    \item \textbf{Recommandation par similarité sémantique} : le pipeline de recommandation combine la génération d'embeddings (768 dimensions via \tech{mpnet-base-v2}), la recherche vectorielle ultra-rapide dans Qdrant (algorithme HNSW, $\sim$12ms), et un scoring multicritères pondéré. Le cache Redis à trois niveaux de matching assure un temps de réponse de $\sim$2ms pour 75\% des requêtes.

    \item \textbf{Ranking multicritères de livreurs} : le module combine le filtrage spatial par ellipse (formule de Haversine), la pondération AHP (avec vérification de cohérence), et le classement TOPSIS pour produire un ranking objectif en $\sim$23ms. Le module est entièrement \textit{stateless}, ce qui garantit des performances constantes indépendamment de la charge.

    \item \textbf{Infrastructure de monitoring} : la stack ELK (Elasticsearch, Kibana, APM Server) avec sécurité xpack offre une observabilité complète : traçage distribué des requêtes, centralisation des logs structurés, et monitoring des workers Celery via Flower. Le déploiement conditionnel par profil Docker Compose permet une activation à la demande.
\end{enumerate}

\section*{Compétences mobilisées}

Ce projet a permis de mettre en pratique les concepts fondamentaux de l'administration réseaux et systèmes :

\begin{itemize}
    \item \textbf{Conteneurisation} : construction d'images Docker optimisées, orchestration de 10+ services via Docker Compose, gestion des réseaux et volumes.
    \item \textbf{Architecture distribuée} : communication inter-services par réseau bridge, files d'attente asynchrones (Celery + Redis), traçabilité par correlation ID.
    \item \textbf{Sécurité} : authentification xpack sur la stack ELK, conteneur d'initialisation automatique des utilisateurs, exécution non-root dans les conteneurs.
    \item \textbf{Monitoring et observabilité} : collecte centralisée des logs, traçage distribué APM, métriques d'infrastructure.
\end{itemize}

\section*{Perspectives d'amélioration}

Plusieurs axes d'évolution ont été identifiés pour les versions futures :

\begin{itemize}
    \item \textbf{Orchestration avancée} : migration vers Kubernetes pour le scaling automatique et la haute disponibilité.
    \item \textbf{Fine-tuning des modèles} : entraînement de modèles de sentiment spécialisés sur les avis du domaine automobile pour améliorer la précision.
    \item \textbf{Enrichissement du ranking} : intégration de critères dynamiques (historique de livraison en temps réel, disponibilité confirmée) dans le module de ranking.
    \item \textbf{Interface utilisateur} : développement d'un tableau de bord web pour la visualisation des recommandations et des classements.
\end{itemize}

\elegantsep

\begin{successbox}
Le système AR\_AS démontre qu'il est possible de construire une plateforme d'IA performante et observable en s'appuyant sur des technologies open-source modernes et des pratiques d'infrastructure rigoureuses. L'architecture modulaire adoptée facilite l'évolution et la maintenance du système à long terme.
\end{successbox}
